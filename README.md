...
Quality gates for developers — security, secrets, licenses, lint, types, dead code, and accessibility. One command, one unified actionable report.
...
**Auto-detection:** lint/type/dead-code gates light up automatically when the tool exists in your project — nio never downloads heavy tooling itself. Security gates ship with nio and always work.
...
npx nio <gate>         # run one gate (eslint, secrets, typecheck, ...)
...
Every run writes to `test-results/nio/`:
...
Then for accessibility, point nio at your running dev server:
...
- uses: actions/upload-artifact@v4
  ...
  with:
    name: nio-report
    path: test-results/nio/
```

Exit code is non-zero only on blocking-gate failure. Parse `test-results/nio/latest.json` for per-finding detail:
...
## What each security gate catches
