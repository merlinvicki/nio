...
export default {
	...
	async run({ root }) {
		...
		// Non-zero exit with no per-test failures = startup/config/browser error
		if (result.code !== 0 && findings.length === 0) {
			const errLine = result.stderr.split('\n').find(l => l.trim())
				|| result.stdout.split('\n').find(l => l.trim() && !l.startsWith('{'))
				|| 'playwright exited non-zero with no test failures reported';
			findings.push({
				file: 'playwright', line: null, rule: 'playwright/run-error', severity: 'error',
				message: errLine.slice(0, 200),
				fix: 'Check webServer config in playwright.config — run: npx playwright install --with-deps',
			});
		}
		...
		].filter(Boolean).join(', ') || result.stderr.split('\n').find(l => l.trim()) || result.stdout.trim().split('\n').slice(-2).join(' ');
